...
const char* archivoKeyframes = "KeyFrames/keyframes.txt";
...
//NEW// Keyframes
float posXavion = 2.0, posYavion = 20.0, posZavion = 0;
...
typedef struct _frame
{
	...
	// ======================================= guardar ahora el mov de las alas
	float giroAlas;
	float giroAlasInc;
}FRAME;
...
{
	std::ifstream archivo(archivoKeyframes);

	if (!archivo.is_open())
	{
		printf("No existe archivo de keyframes. Se creara al guardar.\n");
		return;
	}

	if (archivo.peek() == std::ifstream::traits_type::eof())
	{
		printf("El archivo existe, pero esta vacio.\n");
		archivo.close();
		return;
	}
	...
	archivo.close();
	printf("\n\n\tKeyframes cargados desde archivo: %d\n", FrameIndex);
}

void guardarKeyframesArchivo()
{
	std::ofstream archivo(archivoKeyframes);

	if (!archivo.is_open())
	{
		printf("No se pudo abrir el archivo para guardar keyframes\n");
		return;
	}
	...
	for (int i = 0; i < FrameIndex; i++)
	{
		archivo << KeyFrame[i].movAvion_x << " "
			<< KeyFrame[i].movAvion_y << " "
			<< KeyFrame[i].giroAvion << " "
			<< KeyFrame[i].giroAlas << std::endl;
	}

	archivo.close();
	printf("Keyframes guardados en archivo\n");
}
...
void saveFrame(void) //tecla L
{
	if (FrameIndex >= MAX_FRAMES)
	{
		printf("Ya no se pueden guardar mas frames\n");
		return;
	}
...
int main()
{
	...
	Ala_M_der = Model();
	Ala_M_der.LoadModel("Models/ala_der.obj");
	Ala_M_izq = Model();
	Ala_M_izq.LoadModel("Models/ala_izq.obj");
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = glm::rotate(model, (giroAlas)*toRadians + (170.0f * toRadians), glm::vec3(0.0f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Ala_M_der.RenderModel();

		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, 0.0f, -0.3f));
		model = glm::rotate(model, ((-giroAlas)*toRadians), glm::vec3(0.0f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Ala_M_izq.RenderModel();
...
void inputKeyframes(bool* keys)
{
	...
	//	}
	//}
	// Movimiento manual para crear keyframes
	if (keys[GLFW_KEY_RIGHT])
	{
		movAvion_x += 0.05f;
		printf("X: %f, Y: %f, Giro: %f, Aleteo: %f\n", movAvion_x, movAvion_y, giroAvion, giroAlas);
	}
